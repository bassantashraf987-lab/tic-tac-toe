...
def new_game():
    ...
    for row in range(3):
         for column in range(3):
              board[row][column].config(text="",foreground=color_red, background= color_gray)
...
label = tkinter.Label(frame, text= current_player+"'s turn" , 
                      font=("consolas", 20), background=color_gray,
...
for row in range(3):
        for column in range (3):
            board[row][column] = tkinter.Button(frame, text="", font=("consolas",50 , "bold"),
                                            background=color_gray, foreground=color_red, width=4, height=1, 
